...
ngerman, english,% Language: last one is the main language used in the document
...
ingenieurin, % ingenieur, % for title page ... der Würde einer/eines Doktor-Ingenieurin/Ingenieurs
...
\usepackage{xurl} %break links in bibliography
...
\title{Title of the Thesis\\with line breaks\\for better formatting} % set proper title case: https://titlecaseconverter.com/
% \subtitle not needed for english title, see https://www.f07.uni-stuttgart.de/forschung/promotion-und-habilitation/promotion/dokumente/Titelblatt-Dissertation-1Einreichung_2Drucklegung.pdf (S. 25)

\author{Jane Doe,~M.Sc.}
\birthplace{Birthplace}

\examiner{Univ.-Prof.~Dr.-Ing.~Main~Examiner}
\coexaminer{Univ.-Prof.~Dr.-Ing.~Co~Examiner}
...
%\newacronym{ISW}{ISW}{Institute for Control Engineering of Machine Tools and Manufacturing Units}
%\newacronym{FE}{FE}{Finite Elements}

% Glossary entries, also needs \input{settings/glossaries}, use in text as \gls{Q}
%\glsxtrnewsymbol[description={Covariance of the system noise of the Kalman filter}]{Q}{\ensuremath{\bm Q}}
%\glsxtrnewsymbol[description={Covariance of the measurement noise of the Kalman filter}]{R}{\ensuremath{R}}
...
\frontmatter
% title page according to https://www.f07.uni-stuttgart.de/forschung/promotion-und-habilitation/promotion/dokumente/Titelblatt-Dissertation-1Einreichung_2Drucklegung.pdf
\maketitle
\onehalfspacing % line spacing: 1,5

\addchap{Preface}
% Formula example
\(a_x + b_y \sum_i^N dx + \int\) \footnote{This is a formula.}

% Example SI unit package
\SI{3.14159265e1}{kg.m/s^2} \cite{colu92}


%Example for referencing a glossary entry
%Matrices \gls{Q} and \gls{R}
%also in equations: $\gls{Q}$
%
%Example for referencing an acronym:
%\ac{ISW}.

\cleardoublepage
% two versions required: Kurzfassung in German and Abstract in English, see 
%"Richtlinien für die äußere Form der Veröffentlichung von Dissertationen" 
% in https://elib.uni-stuttgart.de/items/6ff82d8a-b81e-4e55-99ac-345cbdc91e13
\addchap{Kurzfassung}
\addchap{Abstract} 
...
%\printglossary[type=\acronymtype, style=alttree, title={Acronyms}]
%
%\cleardoublepage
%\printglossary[type=symbols,style={symbunitlong}, title={List of Symbols}]
...
% Own chapters: further problem analysis, solution, validation
